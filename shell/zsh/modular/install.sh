...
show_recommended_tools() {
    ...
    case "$os" in
        ...
        wsl|linux)
            ...
            echo ""
            echo "  Install Java 17 JDK (required for React Native / Expo):"
            echo "    sudo apt install -y openjdk-17-jdk"
            echo "    sudo update-alternatives --set java /usr/lib/jvm/java-17-openjdk-amd64/bin/java"
            echo "    sudo update-alternatives --set javac /usr/lib/jvm/java-17-openjdk-amd64/bin/javac"
            echo ""
            echo "  Install Android Studio (via snapd):"
            echo "    sudo apt install -y snapd"
            echo "    sudo snap install android-studio --classic"
            echo ""
            echo "  Install productivity apps (via snap - recommended in 2026):"
            echo "    # Microsoft Teams (community-maintained, official MS dropped Linux app)"
            echo "    sudo snap install teams-for-linux"
            echo "    # Telegram (official snap from Telegram)"
            echo "    sudo snap install telegram-desktop"
            echo "    # Postman (official snap)"
            echo "    sudo snap install postman"
            echo "    # MongoDB Compass (official snap)"
            echo "    sudo snap install mongodb-compass"
            echo ""
